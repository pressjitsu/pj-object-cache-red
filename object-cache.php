...
if ( class_exists( 'Redis' ) ) {
	...
	/**
	 * Deletes the alloptions cache value whenever an option is modified. Fix a race condition in alloptions caching.
	 *
	 ...
	 */
	function pj_maybe_clear_alloptions_cache( $option ) {
		if ( ! wp_installing() ) {
			$options = wp_load_alloptions(); //alloptions should be cached at this point
			if ( isset( $options[ $option ] ) ) { //only if option is among alloptions
				wp_cache_delete( 'alloptions', 'options' );
			}
		}
	}
	add_action( 'added_option',   'pj_maybe_clear_alloptions_cache' );
	add_action( 'updated_option', 'pj_maybe_clear_alloptions_cache' );
	add_action( 'deleted_option', 'pj_maybe_clear_alloptions_cache' );
	...
	if ( function_exists( 'wp_using_ext_object_cache' ) ) {
		...
		wp_die( 'Redis class not available.' );
	}
}